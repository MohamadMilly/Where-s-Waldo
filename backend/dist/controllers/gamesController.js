import { createSession, endSession, getSession, } from "../services/sessionService.js";
import { sign, verify } from "../utils/auth/jwt.js";
import { HttpError } from "../errors/HttpError.js";
import { checkCoords, checkWin } from "../services/gameService.js";
import { createScore } from "../services/scoresService.js";
/* The start controller is responsible for creating a session and a token and send it back to the user,
 */
export const startGamePost = async (req, res, next) => {
    try {
        const session = await createSession();
        const token = sign({ sessionId: session.sessionId }, { expiresIn: "1h" });
        res.json({
            sessionToken: token,
        });
    }
    catch (err) {
        next(err);
    }
};
/* For ending a game , we should grab the session token, the scene Id and characters with their coords , then i am going to decode the token after that i am going to fetch the scene with characters ids
then i am going to validate the answers , if they are correct i will end the session and calculate the time then create a score and tie it to the user

*/
export const endGamePost = async (req, res, next) => {
    const { sessionToken, sceneId, characters } = req.body;
    const currentUser = req.currentUser;
    try {
        if (!currentUser) {
            throw new HttpError(401, "Current user object required");
        }
        const { sessionId } = verify(sessionToken);
        let session = await endSession(sessionId);
        if (!session) {
            throw new HttpError(400, "Cannot end a session that does not exist.");
        }
        const sessionDuation = new Date(session.endTime).getTime() -
            new Date(session.startTime).getTime(); // in melliseconds
        const allCoordsCorrect = await checkWin(sceneId, characters);
        const hasWon = sessionDuation / 1000 < 60 * 60 * 2 && allCoordsCorrect;
        const score = await createScore(currentUser.id, sceneId, sessionDuation, hasWon, {});
        res.json({
            score: score,
            hasWon: true,
        });
    }
    catch (err) {
        next(err);
    }
};
export const verifyCharacterCoordsPost = async (req, res, next) => {
    const { characterId, coords, sessionToken } = req.body;
    try {
        const { sessionId } = verify(sessionToken);
        const session = await getSession(sessionId);
        if (!session) {
            throw new HttpError(400, "SessionToken missing");
        }
        const { isValid, character } = await checkCoords(characterId, coords);
        res.json({
            isValid: isValid,
            character: character,
        });
    }
    catch (err) {
        next(err);
    }
};
