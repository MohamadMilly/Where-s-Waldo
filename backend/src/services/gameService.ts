...
export async function checkCoords(
  characterId: number,
  coords: number[],
): Promise<{ isValid: boolean; character: Character | null }> {
...
export async function checkWin(
  sceneId: number,
  charactersWithCoords: Character[],
): Promise<boolean> {
  const scene = (await getScene(
    { id: sceneId },
    { include: { characters: true } },
  )) as Required<Scene> | null;
  if (!scene) {
    throw new HttpError(404, "Scene does not exist");
  }
  return scene.characters.every((character: Character) =>
    charactersWithCoords.some((answerCharacter) =>
      validateCharacterCoords(character.coords, answerCharacter.coords),
    ),
  );
}
